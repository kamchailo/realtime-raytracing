...
void VkApp::myloadModel(const std::string& filename, glm::mat4 transform)
{
    ...
    // are emitters, and scale the emission up by a factor of 5. --- cancelled this ---  The
    ...
    {
        if (meshdata.materials[i].emission != vec3(0)) // are emitters
        {
            meshdata.materials[i].emission *= 5.0f;
        }
    }
