...
void VkApp::destroyAllVulkanResources()
{
    ...
    // Destroy denoise descriptor (Buffers)
     ...
     // Destroy from createScDescriptorSet();
     m_scDesc.destroy(m_device);

     // Destroy from createScanlineRenderPass()
     vkDestroyRenderPass(m_device, m_scanlineRenderPass, nullptr);
     vkDestroyFramebuffer(m_device, m_scanlineFramebuffer, nullptr);

     // Destroy from createMatrixBuffer();
     // and          createObjDescriptionBuffer();
     m_objDescriptionBW.destroy(m_device);
     m_matrixBW.destroy(m_device);
     
     // Destroy Loaded Objects
     for (auto text : m_objText) text.destroy(m_device);
     for (auto desc : m_objData)
     {
         desc.vertexBuffer.destroy(m_device);
         desc.indexBuffer.destroy(m_device);
         desc.matColorBuffer.destroy(m_device);
         desc.matIndexBuffer.destroy(m_device);
     }        

     // Destroy from createPostPipeline()
     vkDestroyPipelineLayout(m_device, m_postPipelineLayout, nullptr);
     vkDestroyPipeline(m_device, m_postPipeline, nullptr);

     // Scanline Buffer and Post Descriptor
     // Destroy from createScBuffer();
     // and          createPostDescriptor();
...
#ifdef GUI
    // Cleanup ImGui Vulkan related data
    ImGui_ImplVulkan_Shutdown();
    // Cleanup ImGui GLFW related data
...
void VkApp::createInstance(bool doApiDump)
{
    ...
    std::vector<const char*> extensions(reqGLFWextensions, reqGLFWextensions + countGLFWextensions);
    ...
    {
        reqInstanceExtensions.push_back(ext);
        std::printf("- %s\n", ext);
    }
    ...
    for (auto physicalDevice : physicalDevices) {
        ...
        */
        ...
        if (GPUproperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
        {
            ...
            printf("- - Chosen m_physicalDevice: %p %s\n", m_physicalDevice, GPUproperties.deviceName);
        }
    }

    assert(m_physicalDevice, "No supported physical device.");
}
...
void VkApp::chooseQueueIndex()
{
    ...
    // Verify that your search choose the correct queue family.
    ...
    m_graphicsQueueIndex = std::numeric_limits<uint32_t>::max();

    for (int i = 0; i < queueProperties.size(); ++i)
    {
        auto currentQueueFlags = queueProperties[i].queueFlags;
        if (requiredQueueFlags == (requiredQueueFlags & currentQueueFlags))
        {
            m_graphicsQueueIndex = i;
            std::printf("- supported queue %d\n", i);
        }
    }

    std::printf("- - Chosen queueIntex: %d\n", m_graphicsQueueIndex);

    assert(m_graphicsQueueIndex != std::numeric_limits<uint32_t>::max());
}
...
void VkApp::getSurface()
{
    ...
    auto resultCreateWindowSurface = glfwCreateWindowSurface(m_instance, app->GLFW_window, nullptr, &m_surface);
    assert(resultCreateWindowSurface == VK_SUCCESS);

    auto resultGetPhysicalDeviceSurfaceSupport = vkGetPhysicalDeviceSurfaceSupportKHR(m_physicalDevice, m_graphicsQueueIndex,
                                         m_surface, &isSupported);
    std::printf("\nIs this physical device support draw on screen: %d\n", isSupported);

    // @@ Verify VK_SUCCESS from both the glfw... and the vk... calls.
    assert(resultGetPhysicalDeviceSurfaceSupport == VK_SUCCESS);
...
void VkApp::createSwapchain()
{
    ...
    if (formatsCount != 0) {
        ...
        vkGetPhysicalDeviceSurfaceFormatsKHR(m_physicalDevice, m_surface, &formatsCount, formats.data());
    }
    ...
    //std::printf("list of formats/color-spaces:\n");
    ...
    VkResult result;
    result = vkCreateSwapchainKHR(m_device, &_i, nullptr, &m_swapchain);
    // @@ Verify VK_SUCCESS
    assert(result == VK_SUCCESS);
    ...
    for (uint i = 0; i < m_imageCount; i++) {
        ...
        result = vkCreateImageView(m_device, &createInfo, nullptr, &m_imageViews[i]);
        assert(result == VK_SUCCESS);
    }
                                 ...
                                 VkMemoryPropertyFlags properties, uint mipLevels)
{
    ...
    VkResult result;
    result = vkCreateImage(m_device, &imageInfo, nullptr, &myImage.image);
    assert(result == VK_SUCCESS);
...
{  
    ...
    VkResult result;
    result = vkCreateRenderPass(m_device, &renderPassInfo, nullptr, &m_postRenderPass);
    assert(result == VK_SUCCESS);
...
void VkApp::createPostFrameBuffers()
{
    ...
    for(uint32_t i = 0; i < m_imageCount; i++) 
    {
        ...
        VkResult result;
        result = vkCreateFramebuffer(m_device, &_ci, nullptr, &m_framebuffers[i]); 
        assert(result == VK_SUCCESS);
    }
